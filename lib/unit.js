...
Unit.prototype._from = function(amount, code) {
  var unit;
  unit = lodash.find(this.units, function(u) {
    return u.code == code; // Try network specific unit name
  });

  if (!unit) {
    unit = lodash.find(this.units, function(u) {
      return u.kind == code; // Try generic unit kind
    });
  }
...
/**
 * Returns the shortName of the standard unit
 *
 * @returns {string} A shortName
 */
...
/**
 * Returns the shortName of the atomic unit
 *
 * @returns {string} A shortName
 */
...
Unit.prototype.toString = function(code, opts) {
  ...
  var amount = this.to(code).toFixed(unit.precision.short.maxDecimals);

  if (!!opts.includeSeparators) {
    amount = addSeparators(amount, opts.thousandsSeparator, opts.decimalSeparator, unit.precision.short.minDecimals);
  }
