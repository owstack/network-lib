...
URI.Members = ['protocol', 'address', 'amount', 'message', 'label', 'r'];
...
URI.parseWithNetworks = function(protocols, uri) {
  ...
  info.query.protocol = info.protocol;
...
URI.prototype._fromObject = function(obj) {
  ...
  var p = this.address.toString().match(/^.*:/g);
  if (p && p.length > 0) {
    this.protocol = p[0].slice(0, -1);
    this.addressHasProtocol = true;
  } else {
    this.protocol = obj.protocol || this.getLivenetProtocol(); 
  }
...
URI.prototype.toString = function() {
  ...
  return (this.addressHasProtocol ? '' : this.protocol + ':') + this.address + URL.format({query: query});
};
